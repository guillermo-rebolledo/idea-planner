/**
 * Vendored from assistant-ui's shadcn registry
 * (`https://r.assistant-ui.com/base/model-selector.json`), then trimmed.
 *
 * The registry's default export registers the selection into assistant-ui's
 * ModelContext, which its transport sends as `config` in an HTTP request body.
 * This app has no backend — it spawns local CLIs and passes flags — so the
 * default export, its ModelContext registration and the `@assistant-ui/react`
 * import that only it needed are all gone. What is left is `ModelSelectorRoot`
 * and its presentational parts, driven by controlled props (ticket 13).
 *
 * Kept close to the original so it can be diffed against a newer registry
 * copy: the imports are repointed at this app's own `ui/` primitives, and
 * nothing else in the parts below is rewritten.
 */
import {
  useCallback,
  useEffect,
...
function ModelSelectorEffort({
  ...
  if (!efforts?.length) {
    return (
      <div
        ...
        className={cn(
          'flex cursor-default items-center justify-between gap-3 border-t border-border px-3 py-2 opacity-50',
          className
        )}
      >
  ...
  return (
    <div
      data-slot="model-selector-effort"
      className={cn(
        'flex cursor-default items-center justify-between gap-3 border-t border-border px-3 py-2',
        className
      )}
      onKeyDownCapture={(e) => {
        onKeyDownCapture?.(e)
