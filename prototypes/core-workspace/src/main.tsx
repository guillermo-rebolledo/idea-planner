    ...
    <div className={`phase-indicator ${compact ? 'phase-compact' : ''}`} aria-label="Idea phase">
      {phaseNames.map((phase, index) => (
        <React.Fragment key={phase}>
          <span
            className={
              index === currentIndex ? 'phase-current' : index < currentIndex ? 'phase-done' : ''
            }
          >
    ...
    <div className={`activity-row ${open ? 'activity-open' : ''}`}>
      ...
      {open ? (
        <div className="activity-details">
          ...
          <p>
            <ShieldCheck size={13} /> Sensitive values redacted before logging
          </p>
        </div>
      ) : null}
    </div>
    ...
    <div className={`conversation ${condensed ? 'conversation-condensed' : ''}`}>
      <div className="conversation-scroll">
            ...
            <div className="suggestions" aria-label="Suggested responses">
              {suggestions.map((suggestion, index) => (
                <button
                  ...
                  onClick={() =>
                    submitResponse(
                      index === 0 ? 'Yes, keep the files local and visible.' : suggestion
                    )
                  }
                >
                  <span>{index + 1}</span>
        ...
        {submittedResponse ? (
          <>
            ...
            <div className="next-run-state">
              <LoaderCircle size={13} className="spin" /> Wayfinder is continuing from your answer…
            </div>
          </>
        ) : null}
      </div>
      ...
      <div className="composer-wrap">
        <div className="composer">
          ...
          <div className="composer-footer">
            ...
            <div className="composer-actions">
              <UsageIndicator />
              <button className="stop-button" title="Stop current Run">
                <Square size={13} />
              </button>
              <button
                ...
                title="Send response"
                onClick={() => submitResponse(composer)}
              >
                <Send size={15} />
              </button>
            </div>
        ...
        <small className="composer-note">
          Suggested responses send immediately. Use the composer for a custom answer.
        </small>
      </div>
    </div>
  )
...
function ReviewConversation({
  ...
  const copy = {
    spec: {
      ...
      suggestions: [
        'Test the adapter boundary with recorded events',
        'Use end-to-end Electron tests',
        'Propose a balanced testing seam'
      ],
    ...
    tickets: {
      ...
      prompt:
        'Approve the breakdown, or tell me how its granularity or dependencies should change.',
      suggestions: [
        'Approve and create ticket files',
        'Too coarse — split ticket 3',
        'Fix dependencies',
        'Merge tickets 4 and 5'
      ],
    ...
    failure: {
      ...
      prompt:
        'Fix the dependency and review the changed breakdown, or retry unchanged publication.',
      suggestions: [
        'Fix ticket 05 dependency',
        'Show validation details',
        'Retry unchanged publication'
      ],
    ...
    ready: {
      ...
      prompt:
        'You can edit the files directly, refine this Idea through a new Proposal, or implement it in your external Codex or Claude interface.',
      suggestions: [
        'Refine this Idea',
        'Show the implementation frontier',
        'Open Planning Package'
      ],
  ...
  const tickets = [
    ...
    [
      ...
      stage === 'failure' ? 'Invalid blocker: 07' : 'Blocked by 04'
    ]
  ]
  ...
  return (
    <div className="conversation review-conversation">
      <div className="conversation-scroll">
        <div className="history-carryover">
          <Check size={13} /> Earlier Developing conversation remains above
        </div>
        <div className="phase-divider">
          ...
          <div>
            ...
            {stage === 'spec' ? (
              <div className="review-card testing-card">
                ...
                <div>
                  <strong>Testing seam</strong>
                  <small>Required before Spec acceptance</small>
                </div>
                <b>Decision needed</b>
              </div>
            ) : null}
            ...
            {stage === 'tickets' || stage === 'failure' || stage === 'ready' ? (
              <div className="ticket-breakdown">
                <header>
                  <strong>
                    {stage === 'ready' ? 'Implementation frontier' : 'Proposed breakdown'}
                  </strong>
                  <span>{stage === 'ready' ? '2 ready now' : '5 tickets'}</span>
                </header>
                {tickets.map(([number, title, blocker], index) => (
                  <div
                    className={stage === 'ready' && index < 2 ? 'frontier-ticket' : ''}
                    key={number}
                  >
                    <b>{number}</b>
                    <span>
            ...
            {stage === 'ready' ? (
              <details className="implement-guide">
                <summary>
                  <Code2 size={14} /> Implement elsewhere
                </summary>
                <p>
                  Use <code>spec.md</code> and the frontier ticket in Codex or Claude with Matt
                  Pocock’s <code>/implement</code> skill.
                </p>
                <p className="guide-warning">
                  <AlertTriangle size={13} /> The upstream workflow may commit to your current
                  branch. Review it in your chosen TUI or GUI first.
                </p>
              </details>
            ) : null}
            ...
            {stage === 'spec' || stage === 'tickets' || stage === 'failure' ? (
              <button
                className={`phase-action ${stage === 'failure' ? 'retry-action' : ''}`}
                onClick={() => submitResponse(copy.action)}
              >
                {stage === 'failure' ? <AlertTriangle size={14} /> : <Check size={14} />}
                {copy.action}
              </button>
            ) : null}
          </div>
          ...
          <>
            ...
            {stage !== 'ready' ? (
              <div className="next-run-state">
                ...
                {stage === 'failure'
                  ? 'Validating the repaired package…'
      ...
      <div className="composer-wrap">
        <div className="composer">
          <textarea
            ...
            value={composer}
            onChange={(event) => setComposer(event.target.value)}
            placeholder="Discuss, choose a suggestion, or type a custom response…"
            rows={3}
          />
          <div className="composer-footer">
            <RunControls />
        ...
        <small className="composer-note">
          Suggested responses send immediately. Custom responses stay editable until sent.
        </small>
      </div>
    </div>
  )
...
function ArtifactContent({ view, stage = 'developing' }: { view: ArtifactView; stage?: StageKey }) {
  ...
  if (stage === 'spec') {
    return (
      <div className="markdown">
        ...
        <p className="lead">
          A local-first desktop workspace that develops preserved Ideas through deliberate
          AI-assisted planning.
        </p>
        <h2>Accepted behavior</h2>
        <ul>
          <li>One permanent Conversation per Idea.</li>
          <li>AI begins only through explicit user action.</li>
          <li>Planning files remain read-only until Ready.</li>
        </ul>
        <h2>Testing seam</h2>
        <blockquote>
          Decision pending in the Conversation before this Spec can be accepted.
        </blockquote>
      </div>
    )
  }
  ...
  if (stage === 'tickets' || stage === 'failure') {
    return (
      <div className="markdown">
        <p className="eyebrow">
          {stage === 'failure' ? 'STAGED · PUBLICATION FAILED' : 'ACCEPTED SPEC'}
        </p>
        <h1>Implementation tickets</h1>
        <p className="lead">
          Five dependency-aware vertical slices derived from the accepted Spec.
        </p>
        <h2>Publication rule</h2>
        <p className="lead">
          No ticket file becomes authoritative until the complete approved set validates and
          publishes transactionally.
        </p>
        <blockquote>
          {stage === 'failure'
            ? 'Ticket 05 references missing ticket 07. No partial package was published.'
            : 'Review granularity and blocking edges in the Conversation before files are created.'}
        </blockquote>
      </div>
    )
  }
  ...
  if (stage === 'ready') {
    return (
      <div className="markdown">
        ...
        <p className="lead">
          The accepted baseline is durable, portable, and ready for an implementation agent.
        </p>
        <h2>Contents</h2>
        <ul>
  ...
  return (
    <div className="markdown">
      ...
      <ol>
        <li>
          <strong>Developing</strong> — Grill Me or Wayfinder interview
        </li>
        <li>
          <strong>Spec Review</strong> — iterative MVP Spec
        </li>
        <li>
          <strong>Ticket Review</strong> — dependency-aware slices
        </li>
      </ol>
  ...
  return (
    <section className={`artifact-inspector ${compact ? 'artifact-compact' : ''}`}>
      ...
      <div className="artifact-body">
        <ArtifactContent view={view} stage={stage} />
      </div>
      <footer className="artifact-footer">
        <span>
          <Clock3 size={13} /> Snapshot 18
        </span>
        <span>
          {stage === 'ready'
            ? 'Accepted baseline · Editable'
            : `Read-only while ${stageMeta[stage].phase}`}
        </span>
      </footer>
    </section>
  )
  ...
  return (
    <div className={`idea-rows ${compact ? 'idea-rows-compact' : ''}`}>
      ...
      {ideas.slice(0, 3).map((idea) => (
        <button
          key={idea.id}
          className={idea.id === 1 ? 'idea-row active' : 'idea-row'}
          title={idea.title}
        >
          {compact ? (
            <>
...
function VariantA(props: WorkspaceProps) {
    ...
    return (
      <main className="focus-artifact-page">
        <header className="focus-artifact-topbar">
          <button onClick={() => props.setFocusArtifact(false)}>
  ...
  return (
    <main
      className={`variant-a ${leftOpen ? '' : 'left-closed'} ${rightOpen ? '' : 'right-closed'}`}
    >
      ...
      <section className="mail-main">
        <header className="workspace-header">
          <div className="workspace-title">
            <button onClick={() => setLeftOpen(!leftOpen)} title="Toggle inbox">
          ...
          <div className="workspace-tools">
            ...
            <button onClick={() => setRightOpen(!rightOpen)} title="Toggle artifacts">
      ...
      {!leftOpen ? (
        <button className="restore-left" onClick={() => setLeftOpen(true)}>
          <Menu size={17} />
        </button>
      ) : null}
      {!rightOpen ? (
        <button className="restore-right" onClick={() => setRightOpen(true)}>
          <FileText size={17} />
        </button>
      ) : null}
    </main>
  ...
  return (
    <main className="variant-b">
      <aside className="focus-idea-rail">
        ...
        <button className="rail-new" title="New Idea">
          <Plus size={17} />
        </button>
        <IdeaRows compact />
      ...
      <div className="focus-running-dock">
        <div>
          ...
          <span>
            <strong>Offline API field guide</strong>
            <small>Claude is working</small>
          </span>
        </div>
        <button>
          <Square size={12} /> Stop
  ...
  return (
    <main className="variant-c">
      <header className="ledger-topbar">
        ...
        <button className="all-ideas" onClick={() => setShowInbox(!showInbox)}>
          <Inbox size={15} /> All Ideas
        </button>
      </header>
      {showInbox ? (
        <div className="ledger-inbox-popover">
          <header>
            <strong>All Ideas</strong>
          ...
          <div className="search-box">
            <Search size={14} />
            <input placeholder="Search Ideas" autoFocus />
          </div>
          <IdeaRows />
        </div>
      ) : null}
      ...
      <section className={`ledger-body ${showArtifacts ? 'ledger-reviewing' : ''}`}>
        ...
        <aside className="ledger-checkpoints">
          <header>
            <span>
              <FileText size={15} />
              <strong>Document checkpoints</strong>
            </span>
          ...
          {!showArtifacts ? (
            <>
              <button className="checkpoint-card current" onClick={() => setShowArtifacts(true)}>
                ...
                <b>
                  <GitCompareArrows size={13} /> 9 changes
                </b>
              </button>
              ...
              <div className="checkpoint-note">
                <Clock3 size={14} />
                <span>Next checkpoint is created when you continue to Spec Review.</span>
              </div>
            </>
          ) : (
            <ArtifactInspector {...props.artifactProps} compact />
    ...
    <main className={`variant-d ${inboxOpen ? '' : 'inbox-closed'}`}>
      <aside className="focus-mailbox-sidebar">
        <header className="focus-mailbox-brand">
          ...
          <strong>Commonplace</strong>
          <button onClick={() => setInboxOpen(false)} title="Collapse inbox">
            <PanelLeftClose size={15} />
          </button>
        </header>
        <button className="focus-mailbox-new">
          <Plus size={15} /> New Idea
        </button>
        <div className="focus-mailbox-search">
          <Search size={14} />
          <input placeholder="Search Ideas" />
        </div>
        <IdeaRows />
      ...
      <section className="focus-mailbox-main">
        <header className="focus-header">
          <div className="focus-mailbox-title">
            {!inboxOpen ? (
              <button onClick={() => setInboxOpen(true)} title="Open inbox">
                <Menu size={16} />
              </button>
            ) : null}
            <div>
  ...
  return (
    <>
      ...
      <div className="prototype-switcher" aria-label="Prototype variants">
        <button onClick={() => cycle(-1)} title="Previous variant">
          <ArrowLeft size={16} />
        </button>
        <span>
        ...
        <button onClick={() => cycle(1)} title="Next variant">
          <ArrowRight size={16} />
        </button>
      </div>
    </>
