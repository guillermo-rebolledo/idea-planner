...
**What to build:** An approval prompt offers "always allow". Choosing it records a Standing Approval for that Repository, and the same action never prompts again — in this Session or any future one. The user can see and revoke them.
...
Two shapes are needed: a command approval, and blanket edit permission for the Repository — which is what makes Ask mode livable and is why there is no separate auto-edit mode.
...
- [ ] Accepting stores a Standing Approval scoped to the Session's Repository
...
- [ ] Standing Approvals are listed per Repository and individually revocable
- [ ] Approvals never leak across Repositories, including between two clones of the same remote
