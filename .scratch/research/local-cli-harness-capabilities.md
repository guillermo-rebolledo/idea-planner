...
| Mid-turn steering | `turn/steer` with an `expectedTurnId` precondition | A second user frame on stdin, which needs `--input-format stream-json`; folded into the running turn, acknowledged only by the `--replay-user-messages` echo | Both steer. Probed for Claude on 2.1.226 in `claude-steering-on-stdin.md` — the prompt has to travel on stdin for the channel to exist at all, and a frame written after the turn ends starts another one. Queue where the precondition fails. |
